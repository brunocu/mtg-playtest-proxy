...
The system SHALL support an optional alternate/flavor name for a card entry, distinct from the card's real name, so the rendered card can display the flavor name while still being looked up by its real name.
...
- **THEN** the system produces one token entry named "Soldier" with quantity 1
