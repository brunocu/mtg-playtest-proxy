...
The system SHALL render basic land cards with their standard mana symbol overlay by default, SHALL render them without the large mana symbol overlay ("full-art") when the user enables that option, and SHALL exclude basic lands from the output entirely when the user enables the ignore-basic-lands option.

#### Scenario: Full-art basic lands enabled
- **WHEN** the user enables the full-art basic lands option and the decklist includes a Plains
